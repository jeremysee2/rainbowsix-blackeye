...
extern void setLEDRing(int R=0, int G=0, int B=255);
extern void setMotors(int xVal, int yVal);
...
static esp_err_t index_handler(httpd_req_t *req){
    ...
    if  (strncmp(view,"simple", sizeof(view)) == 0) {
        ...
        setLEDRing(0,0,0);
        return httpd_resp_send(req, (const char *)s.c_str(), s.length());
    } else if(strncmp(view,"blackeye", sizeof(view)) == 0) {
        Serial.println("Black eye page requested");
        httpd_resp_set_type(req, "text/html");
        httpd_resp_set_hdr(req, "Content-Encoding", "identity");
        // Turn on LED Ring when actively viewing
        setLEDRing();
        sensor_t * s = esp_camera_sensor_get();
        return httpd_resp_send(req, (const char *)index_blackeye_html, index_blackeye_html_len);
    }
    else  {
