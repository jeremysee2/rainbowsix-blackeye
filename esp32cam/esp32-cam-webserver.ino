...
// UART communication with secondary processor
const int rxPin = 13; // TODO: change
const int txPin = 14;
char motorBuffer[7];

// TODO: Send values to secondary processor through UART
void setMotors(int xVal, int yVal) {
    sprintf(motorBuffer,"%03d,%03d",xVal,yVal);
    Serial2.print(xVal);
    Serial2.print(",");
    Serial2.print(yVal);
    Serial2.print("\n");
...
// LED Ring control
void setLEDRing(int R=0, int G=0, int B=255) {
...
void setLamp(int newVal) {
    if (newVal != -1) {
        ...
        // Change brightness of Blue light on LED Ring
        setLEDRing(0,0,int(255*newVal/100));
...
void setup() {
    ...
    // LED Ring setup
    FastLED.addLeds<WS2812B, ringLEDPin, RGB>(leds, NUM_LEDS);  // GRB ordering is typical
